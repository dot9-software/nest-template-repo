...
import { IsUUIDContainer } from './IsUUIDContainer';
...
describe('IsUUIDContainer', () => {
  ...
  it('should reject values with invalid id', async () => {
    ...
    expect(errors.length).toBeGreaterThan(0);
    errors = await validate(new TestClass({ id: 'not a valid id' }));
    expect(errors.length).toBeGreaterThan(0);
    ...
    errors = await validate(new TestClass({ id: {} }));
    expect(errors.length).toBeGreaterThan(0);
    errors = await validate(new TestClass({ id: { foo: 'bar' } }));
    expect(errors.length).toBeGreaterThan(0);
  })
